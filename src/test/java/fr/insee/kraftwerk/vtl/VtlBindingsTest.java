...
import fr.insee.kraftwerk.parsers.PaperDataParser;
...
public class VtlBindingsTest {
	...
	public void removeNonExistingDataset() {
		assertDoesNotThrow(() -> vtlBindings.getBindings().remove("NOT_IN_BINDINGS"));
	}
	...
	@Test
	public void convertToVtlDatasetTestWithRealExample() {
		SurveyRawData paperData = new SurveyRawData();
		paperData.setVariablesMap(DDIReader.getVariablesFromDDI(
		...
		assertTrue(vtlBindings.getBindings().containsKey("TEST"));

		Dataset test = vtlBindings.getDataset("TEST");
		assertEquals(93, test.getDataStructure().keySet().size());
		assertEquals(2, test.getDataPoints().size());
	}
