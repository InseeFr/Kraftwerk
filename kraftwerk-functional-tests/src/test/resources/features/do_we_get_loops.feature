Feature: Do we save loop data correctly ?
  ...
  Scenario Outline: Do we save external variables loop data in csv file correctly ?
    Given Step 0 : We have some survey in directory "<Directory>"
    When Step 1 : We launch main service file by file
    Then In csv loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <LoopIteration> we should have value "<ExpectedValue>" for field "<FieldName>"
  ...
  Scenario Outline: Do we save collected variables loop data in parquet file correctly ?
    Given Step 0 : We have some survey in directory "<Directory>"
    When Step 1 : We launch main service file by file
    Then In parquet loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <LoopIteration> we should have value "<ExpectedValue>" for field "<FieldName>"
  ...
  Scenario Outline: Do we save external variables loop data in parquet file correctly ?
    Given Step 0 : We have some survey in directory "<Directory>"
    When Step 1 : We launch main service file by file
    Then In parquet loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <LoopIteration> we should have value "<ExpectedValue>" for field "<FieldName>"
