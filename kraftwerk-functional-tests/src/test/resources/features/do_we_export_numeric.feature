Feature: Do we save number format correctly ?
  ...
  Scenario Outline: Do we export large numbers in right format (without E)
    ...
    Then Step 7 : We check that id "<InterrogationId>" has value "<ExpectedValue>" for variable "<VariableName>" in table "<TableName>"
    Examples:
      |Directory                        |InterrogationId |ExpectedValue  |VariableName |TableName  |
