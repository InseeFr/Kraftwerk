Feature: Do we save correctly loop data from Genesis?
  ...
  Scenario Outline: Genesis loop collected data export by questionnaire
    ...
    And We have a collected variable "<VariableName>" in a loop named "<LoopName>" iteration <Iteration> in a document with CampaignId "<QuestionnaireModelId>", InterrogationId "<InterrogationId>" with value "<Value>"
    When We use the Genesis service with campaignId "<QuestionnaireModelId>"
    Then We check if the CSV format is correct
    And In csv loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <Iteration> we should have value "<Value>" for field "<VariableName>"
    And In parquet loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <Iteration> we should have value "<Value>" for field "<VariableName>"
    And Step 2 : We check "<LoopName>" output file has <ExpectedFileLinesCount> lines and <ExpectedVariablesCount> variables
  ...
  Scenario Outline: Genesis loop external data export by questionnaire
    Given Step 0 : We have some survey in directory "<QuestionnaireModelId>"
    And We have a external variable "<VariableName>" in a loop named "<LoopName>" iteration <Iteration> in a document with QuestionnaireModelId "<QuestionnaireModelId>", InterrogationId "<InterrogationId>" with value "<Value>"
    When We use the Genesis service with campaignId "<QuestionnaireModelId>"
    Then We check if the CSV format is correct
    And In csv loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <Iteration> we should have value "<Value>" for field "<VariableName>"
    And In parquet loop file for loop "<LoopName>" for interrogationId "<InterrogationId>" and iteration <Iteration> we should have value "<Value>" for field "<VariableName>"
