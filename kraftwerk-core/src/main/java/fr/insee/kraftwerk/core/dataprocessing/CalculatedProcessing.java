...
public class CalculatedProcessing extends DataProcessing {
	...
	/**
	 * Maximal number of iterations to resolve the order of execution of VTL
	 * expressions.
	 */
	...
	/**
	 * Return the VTL instruction for each calculated variable registered in the
	 * CalculatedVariables object given. FILTER_RESULT variables are added in the
	 * given variables map.
	 * 
	 * @param bindingName The name of the concerned dataset.
	 * @param objects     Objects expected here are: - a CalculatedVariables
	 *                    instance, - the corresponding VariablesMap object (used to
	...
	protected VtlScript generateVtlInstructions(String bindingName, Object... objects) {
		...
		for (String calculatedName : orderedCalculatedNames) {
			...
			/*
			 * If the variable is not registered in the variables map (DDI variables), it
			 * can be a FILTER_RESULT variable created by Lunatic. In this case, Otherwise,
			 * the variable is not created and a warning pops in the log.
			 */
	...
	/**
	 * If the variable name after "FILTER_RESULT_" is recognized in the variables
	 * map, a variable corresponding to the given filter result is added in the map
	...
	private void addFilterResult(String filterResultName, VariablesMap variablesMap) {
		...
		if (variablesMap.hasVariable(correspondingVariableName)) { // the variable is directly found
			...
			log.warn(String.format("No information from the DDI about question or variable named \"%s\".",
					correspondingVariableName));
			log.warn(String.format("\"%s\" has been arbitrarily associated with group \"%s\".", filterResultName,
					group.getName()));
		}
	...
	/**
	 * Return a list of calculated variable names, in such an order that the
	 * evaluation of VTL expressions can be performed.
	 */
	private List<String> resolveCalculated(CalculatedVariables calculatedVariables) {
		...
		// In case sequence break due to counter (should not happen if the lunatic
		// questionnaire given is consistent)
