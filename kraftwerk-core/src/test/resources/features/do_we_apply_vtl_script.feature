Feature: Do we apply a script correctly?
  ...
  Scenario Outline: Do we apply the VTL instructions ?
    ...
    # - numberVariables : expected number of variables in the final table, must include identifiers
    |vtlScript                                 |vtlScript2                             |vtlScript3                        |outputDataset|numberVariables |
    |OUTPUT   := COLEMAN[keep LAST_NAME];|OUTPUT2 := OUTPUT[keep LAST_NAME];     |                                  |OUTPUT2      |3               |
    |COLEMAN2 := COLEMAN[keep LAST_NAME];|PAPER2  := PAPER[keep LAST_NAME];|OUTPUT := union(COLEMAN2, PAPER2);|OUTPUT       |3               |
