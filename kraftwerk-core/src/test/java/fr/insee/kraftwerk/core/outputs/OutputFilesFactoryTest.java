...
class OutputFilesFactoryTest {
    ...
    void createCsv_Test() {
        ...
        doReturn(mockCsvOutputFiles).when(spyOutputFilesFactory).newInstanceOfCsvOutputFiles(any(), any(), any(), any(), any(), any(), any());

        // 2. Launch test
        CsvOutputFiles resultObject = spyOutputFilesFactory.createCsv(any(), any(), any(), any(), any(), any());

        // 3. checks
        verify(spyOutputFilesFactory, times(1)).newInstanceOfCsvOutputFiles(any(), any(), any(), any(), any(), any(), any());
    ...
    void createParquet_Test() {
        ...
        spyOutputFilesFactory = Mockito.spy(new OutputFilesFactory(null));
        doReturn(mockParquetOutputFiles).when(spyOutputFilesFactory).newInstanceOfParquetOutputFiles(any(), any(), any(), any(), any(), any(), any());

        // 2. Launch test
        ParquetOutputFiles resultObject = spyOutputFilesFactory.createParquet(any(), any(), any(), any(), any(), any());

        // 3. checks
        verify(spyOutputFilesFactory, times(1)).newInstanceOfParquetOutputFiles(any(), any(), any(), any(), any(), any(), any());
