...
public class MainDefinitions {
	...
	public void launch_main_filebyfile() throws KraftwerkException {
		...
		deleteDirectory(outDirectory.toFile());
		deleteDirectory(tempDirectory.toFile());
		MainProcessing mp = new MainProcessing(inDirectory.toString(), true, Paths.get(FUNCTIONAL_TESTS_INPUT_DIRECTORY).resolve(campaignName).toString());
		mp.runMain();	
	}
	...
	public void export_results() throws KraftwerkException {
		...
		outputFiles = new CsvOutputFiles(outDirectory, vtlBindings, userInputs.getModes(), userInputs.getMultimodeDatasetName());
	}
	...
	public void count_lines_in_root_tables(int expectedLineCount) throws CsvValidationException, IOException {
		...
		System.out.println("Check output file path : " + outDirectory.resolve(outputFiles.outputFileName(Constants.ROOT_GROUP_NAME)));
	...
	public void check_output_root_table(int expectedLineCount, int expectedVariablesCount) throws IOException, CsvValidationException {
		CSVReader csvReader = CsvUtils.getReader(
				outDirectory.resolve(outDirectory.getFileName() + "_" + Constants.ROOT_GROUP_NAME + ".csv"));
		// get header
	...
	public void checkVariableValue(String idUE,  String expectedValue, String variable, String tableName) throws IOException, CsvValidationException  {
		if (tableName == null || tableName.equals("")) tableName = Constants.ROOT_GROUP_NAME;
		
		// Get reader to read the root table written in outputs
		CSVReader csvReader = CsvUtils.getReader(
				outputFiles.getOutputFolder().resolve(outputFiles.outputFileName(tableName)));
		// get header
		String[] header = csvReader.readNext();
        ...
        while ((line = csvReader.readNext()) != null) {
            if (line[idUEPosition].equals(idUE)) value = line[varPosition];
        }
	...
	private void writeErrorsFile(Path inDirectory, List<KraftwerkError> errors) {
		...
		if (!errors.isEmpty()) {
			try (FileWriter myWriter = new FileWriter(tempOutputPath.toFile(),true)){
				...
				System.out.println((String.format("Error occurred when trying to write text file: %s", tempOutputPath)));
			}
		} else {
